...
case 9:
printf(“\n\n To update\n\n”);
printf(” enter element posion & valuefor updating : “);
scanf(“%d%d”,&p,&y);
update(n,arr,p,y);
ch=menu();
break;
...
int insert_front(int n, int x[10],int y)
{
int i;
printf(” in insert front stub\n”);
for(i=n;i>0;i–)
x[i]=x[i-1];
x[0]=y;
n=n+1;
...
{
int i;
printf(” in insert rear stub\n”);
x[n]=y;
n=n+1;
return(n);
}
int insert_position(int n , int x[10],int p,int y )
{
int i;
printf(” in insert position stub\n”);
for(i=n;i>p;i–)
x[i]=x[i-1];
x[p]=y;
n=n+1;
...
int delete_front(int n, int x[10])
{
int i;
printf(” in delete front stub\n”);
for(i=0;i<n-1;i++)
x[i]=x[i+1];
n=n-1;
return(n);
...
{
int i;
printf( "in traverse forward stub\n");
for(i=0;i<n;i++)
printf("%d" ,x[i]);
printf("\n");
return;
...
{
int i;
printf(" in traverse backward stub\n");
for(i=n-1;i>=0;i--)
printf("%d" ,x[i]);
printf("\n");
return;
